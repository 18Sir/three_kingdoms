  ...
  <div>
    <div class="card">
      ...
      >
        ...
        <Column field="title" header="帖标题" sortable style="min-width: 10rem">
          <template #body="{ data }">
    ...
    >
      ...
      <div class="field">
        ...
        <el-radio-group
          ...
          @change="changeStatus"
          size="large"
          :disabled="data.result.includes('删帖') && !isSave"
        >
          <el-radio-button value="不违规">不违规</el-radio-button>
          <el-radio-button value="打回">打回</el-radio-button>
          <el-radio-button value="仅删帖">仅删帖</el-radio-button>
          <el-radio-button value="删帖并封禁用户"
            >删帖并封禁用户</el-radio-button
          >
        </el-radio-group>
      </div>

      <div class="field" v-if="data.result == '删帖并封禁用户' && isSave">
...
import {
  getAlljuBaoMsgService,
  deletejuBaoMsgsService,
  deletejuBaomsgService,
  updatejuBaomsgService,
} from "@/api/juBao";
...
const resultsOptions = [
  ...
  { label: "打回", type: "primary", status: 1 },
  { label: "仅删帖", type: "warning", status: 2 },
  { label: "删帖并封禁用户", type: "danger", status: 3 },
];
...
const resultLabels = ["未处理", "不违规", "打回", "仅删帖", "删帖并封禁用户"];
...
//获取帖子内容
const getMsgDatas = () => {
  getAlljuBaoMsgService().then((res) => {
...
const saveData = () => {
  ...
  if (data.value.result.includes("封禁")) {
    ...
    blockParams.reason =
      '帖子<span style="color:blue;">【' +
      data.value.msg.title +
      "】</span>" +
      data.value.blockReason;
    blockParams.uid = data.value.beUid;
  }
  submitted.value = true;
  if (isSave.value) {
    updatejuBaomsgService(params).then((res) => {
        ...
        if (params.result.includes("封禁")) {
          blockUserService(blockParams).then((res) => {
            if (res.code.endsWith("OK")) {
              //封禁成功后向举报用户反馈
              let content =
                '你好，你举报的帖子<span style="color:blue;">【' +
                data.value.msg.title +
                "】</span>,理由：" +
                data.value.reason +
                "，经核实确认违规，该帖现已删除，并对帖主进行封禁" +
                data.value.blockTime.name +
                "的处罚，感谢你对社区做出的贡献";
...
const deleteData = () => {
  ...
  deletejuBaomsgService(data.value.jid).then((res) => {
    ElMessage.success(res.msg);
    getMsgDatas();
  });
...
const deleteSelectedDatas = () => {
  ...
  deletejuBaoMsgsService(jids).then((res) => {
    ElMessage.success(res.msg);
    getMsgDatas();
  });
